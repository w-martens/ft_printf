...
/*                                                   +#+                      */
/*   Created: 2020/06/02 17:19:51 by wmartens      #+#    #+#                 */
/*   Updated: 2020/06/02 17:19:52 by wmartens      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int		hex_len(t_printf *p_s, unsigned int n)
{
	...
	ret = 0;
	if ((n == 0 && p_s->prec > 0) || (n == 0 && p_s->prec_on == 0))
		ret++;
	while (n > 0)
	{
		n /= 16;
...
void	hex_print(t_printf *p_s, unsigned int n)
{
	...
	if (n == 0 && p_s->prec != 0 && p_s->spec != 'p')
	{
		write(1, &hex[0], 1);
		p_s->ret_cnt++;
	}
	else if ((n == 0 && p_s->spec == 'p') || (n == 0 && p_s->prec_on == 0))
	{
		write(1, &hex[0], 1);
		p_s->ret_cnt++;
	}
